...
async function main() {
    ...
    for (const [bi, section] of boards.entries()) {
      for (const row of section.querySelectorAll(".cr-board-row:not(.cr-board-head)")) {
        ...
        for (let i = 1; i < bottoms.length; i++) {
          ...
          if (delta > 1.5) {
            baselineIssues.push({ board: bi, name: bottoms[0].text, against: bottoms[i].text, delta });
          }
        }
      }
    }
  ...
  const fails = [];
  if (report.baselineIssues.length) fails.push(JSON.stringify(report.baselineIssues));
  if (report.nameLeftDelta != null && report.nameLeftDelta > MAX_COLUMN_DELTA_PX) {
    fails.push(`name column left delta ${report.nameLeftDelta}`);
  }
  if (report.firstStatLeftDelta != null && report.firstStatLeftDelta > MAX_COLUMN_DELTA_PX) {
    fails.push(`first stat column left delta ${report.firstStatLeftDelta}`);
  }
  if (fails.length) {
    console.error("FAIL", fails);
    process.exit(1);
  }
  console.log("PASS crease scorecard alignment", JSON.stringify(report));
}
