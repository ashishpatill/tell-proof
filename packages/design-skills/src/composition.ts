...
export function displaySizeFor(siteKind: SiteKind, lean: AestheticLean, density: Density): number {
  ...
  // Care pathway: quieter display — stage columns own the fold (eye > shout).
  if (siteKind === "care-pathway") px = 40;
  ...
  if (siteKind === "care-pathway") return Math.max(36, Math.min(44, px));
