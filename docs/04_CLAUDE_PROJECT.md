...
- The demo shows a real capture → diagnose → art-direct → reconcile loop, runnable from Cursor via MCP.
...
- FR6 Accept voice (or text preset) art-direction and re-propose a redesign direction + token/CSS diff.
- FR7 Draft a reconciliation diff for a chosen finding or full redesign; never auto-apply.
...
- NFR4 Accessibility floor met by the app itself (WCAG AA, focus-visible, non-color verdict encoding,
  reduced-motion).
...
(Gemini reasoning + voice direction) → `redesign` (diff gen behind an interface) → `mcp`
...
+ voice art-direction + reconciliation, in Cursor.** No shipped product occupies it.
...
**Stretch:** FR6–FR7 (voice art-direction + full redesign diff + Apply in Cursor); live URL capture
(any public URL); Playwright state-probe thumbnails in evidence panels.

**Explicit cut line:** if behind on Day 2, drop `packages/redesign` full-diff generation and live
URL capture; ship capture → fingerprint → detect → reason → Tell Report + before/after seam on the
committed fixture artifact. That still wins the room.
...
- Tell captures a product's REAL RENDERED UI (Playwright/CDP), builds a deterministic design
  ...
  drift vs intentional, accepts voice art-direction, and drafts redesign diffs callable from Cursor
  via MCP.
...
- Stack: TypeScript, Playwright, Next.js 14, Tailwind, Gemini for taste, Anthropic for redesign
  diffs, MCP (stdio), zod, vitest, pnpm workspaces.
...
How to help:
- Keep the deterministic core (capture + fingerprint + detection) free of LLM/network calls. LLM is
  only for taste verdicts, voice direction parsing, and redesign diffs.
...
4. **Before/after (40s).** Drag the reveal seam. Left: the bland fixture. Right: Tell's proposed
   editorial direction (warm paper, Instrument Serif display, restrained accent). "This is the
   redesign you've needed for a year."
5. **Voice + reconcile (40s).** Hold mic: "Warmer, more editorial." Direction updates; diff appears
   → `Apply in Cursor`. Close: "Tell runs on itself: zero tells."
