...
4. **Reconciliation / Diff** — the proposed redesign patch.
5. **Voice Director** — mic + presets + active direction tag.
...
layout. Progress mono readout: `loading page…` → `reading computed styles…` → `detecting tells…` →
`reasoning…`. Reduced-motion: static "Capturing…" label, no flash.
...
`Apply in Cursor`. Below diff: mini seam preview showing the fix effect. CTA naming consistent:
`Draft fix` → `Apply in Cursor` → toast `Fix ready in Cursor`.

### 3.9 Voice Director (standalone frame)

Horizontal strip, `e1`. Left: circular mic button (terra when listening, pulsing ring). Center:
transcript text or placeholder. Right: four preset chips. Show both idle and listening states.
