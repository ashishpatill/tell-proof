...
import { ingestReportFile } from "./ingest.js";
...
export async function watchInbox(opts: WatchOptions = {}): Promise<void> {
  ...
  watcher.on("add", async (file) => {
    try {
      const ep = await ingestReportFile(file, { source: "watch" });
      console.log(`[ingest] ${ep.episode_id} reward=${ep.reward.total} url=${ep.url}`);
      if (autoConvert) {
        const stats = await convertAll(home);
        console.log(`[convert] sft=${stats.sft} dpo=${stats.dpo} corrections=${stats.corrections} dropped=${stats.dropped}`);
      }
    } catch (err) {
