/**
 * Parameter → frame mapper for interactive motion.
 * Maps scroll/pointer progress to discrete frames with smoothDamp tracking
 * (avoids lerp stickiness on direction reversals).
 */
...
const shortestCircularDelta = (from: number, to: number, frameCount: number) => {
...
export function createFrameAnimator(options: FrameAnimatorOptions): FrameAnimator {
