...
type Props = {
  ...
  /** Kept for call-site stability; finding selection lives in the critic rail, not as seam pins. */
  findings: Finding[];
...
export function BeforeAfterSeam({
  ...
  return (
    ...
    >
      ...
      {/* Finding selection lives in the critic rail — no crosshair pins on the proof surface. */}
      ...
      <button
        ...
        onKeyDown={onKeyDown}
        className="seam-handle absolute top-1/2 z-30 -translate-x-1/2 -translate-y-1/2 cursor-ew-resize rounded-sm border border-accent bg-surface-raised px-2 py-3 shadow-signal"
        style={{ left: `${seam}%` }}
      >
        <span aria-hidden className="block h-8 w-1 rounded-full bg-accent" />
      </button>
